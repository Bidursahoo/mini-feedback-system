...
export default function FeedBackForm() {
  ...
  return (
    ...
    {localStorage.getItem("authTocken")?(
      <>
      <h1 className="text-center text-warning">Welcome {localStorage.getItem("name")}</h1>
      <h1 className="text-center">Submit Your FeedBack</h1>
      <div className="text-center">
      <button onClick={handleClick} className="btn btn-danger ml-2">LogOut</button>
      </div>
      ...
      <>
      <h1 className="text-danger text-center">Unautorized Access Not Allowed</h1>
      <div className="text-center">
      <button className="btn btn-success" onClick={()=>{navigate("/")}} type="submit">LogIn</button>
      </div>
      </>
    }
    </>
